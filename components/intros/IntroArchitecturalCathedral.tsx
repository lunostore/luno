...
export function IntroArchitecturalCathedral({ onComplete }: { onComplete: () => void }) {
  ...
  const [introImages, setIntroImages] = useState<string[]>(["/logo.png"]);
  ...
  useEffect(() => {
    getSiteSettings()
      .then((data) => {
        if (data) {
          const imgs = [...(data.heroImagesDark || []), ...(data.heroImagesLight || [])].filter(Boolean);
          if (imgs.length > 0) setIntroImages(imgs);
        }
      })
      .catch(console.error);
  }, []);
  ...
  const currentIntroImg = introImages[activeImgIndex % introImages.length] || "/logo.png";
