...
export function IntroAppleMinimal({ onComplete }: { onComplete: () => void }) {
  const [step, setStep] = useState<"dark" | "l" | "u" | "n" | "o" | "merge" | "slogan" | "exit">("dark");

  useEffect(() => {
    const t0 = setTimeout(() => setStep("l"), 300);        // 0.3s: L appears
    const t1 = setTimeout(() => setStep("u"), 800);        // 0.8s: U appears
    const t2 = setTimeout(() => setStep("n"), 1300);       // 1.3s: N appears
    ...
    <AnimatePresence>
      {step !== "exit" ? (
        ...
        >
          ...
          {/* Letter Sequential Reveal: L -> U -> N -> O */}
