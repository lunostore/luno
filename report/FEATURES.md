...
- **شاشة القيادة والمؤشرات الحيوية (KPIs Overview Dashboard)**:
  ...
  - **رادار الحملات الترويجية ومؤشر الشيك أوت (Campaigns & Abandonment Radar)**: شريط تنبيهي ذكي يوضح فورياً عدد الحملات الإعلانية النشطة التي تسجل زيارات على المنتجات، وعدد الزوار الذين وصلوا لصفحة الدفع ولم يكملوا الطلب.
...
- **🎯 رادار الحملات الإعلانية وتتبع المنتجات (Ad Campaign Radar & Attribution Tracker)**:
  - تتبع دقيق لمعلمات الحملات الإعلانية الممولة (`utm_campaign`, `utm_source`, `utm_medium`).
  - رصد النقرات والزيارات الفعلية لكل إعلان موجه لمنتج معين (**Actual Visits per Product**).
  - جدول تفصيلي في لوحة الإدارة يوضح: المنصة (TikTok, Instagram, Facebook, Google..)، اسم المنتج وصورته وسعره، عدد الزيارات، الزوار الفريدين، من وصلوا لصفحة الدفع، والمبيعات المكتملة مع نسبة التحويل (`Conversion Rate %`).
  - زر فلترة سريع ("عرض زوارها") لعزل جلسات زوار الحملة مباشرة في جدول الجلسات الحي.

- **🔗 مُولّد روابط الحملات الإعلانية الذكي (Campaign URL Builder)**:
  - نافذة منبثقة تفاعلية في صفحة التحليلات لإنشاء روابط إعلانات ممولة احترافية خلال ثوانٍ.
  - اختيار المنتج من قائمة منتجات المتجر مع السعر والصورة.
  - أزرار سريعة للمنصات الإعلانية: `TikTok`, `Instagram`, `Facebook`, `Snapchat`, `Google`, `WhatsApp`.
  - أزرار بنقرة واحدة لنسخ الرابط الجاهز وتجربته فورياً.

- **🛒 تتبع أقصى مرحلة وصل إليها الزائر (Furthest Funnel Stage Tracking)**:
  - نظام أوزان تصاعدي يمنع ضياع معرفة وصول الزائر لصفحة الدفع حتى لو تراجع وتصفح منتجات أخرى:
    1. تصفح عام (`browse`)
    2. مشاهدة منتج (`product`) مع حفظ اسم المنتج المشاهد
    3. سلة المشتريات (`cart`)
    4. صفحة الدفع والشيك أوت (`checkout`)
    5. إتمام الشراء بنجاح (`order_success`)
  - **إذا دخل الزائر صفحة الدفع (`/checkout`) ثم عاد للرئيسية، يظل مسجلاً في لوحة التحكم كـ "وصل لصفحة الدفع (Checkout 🛒💳)"**.
  - شارات ملونة توضح مرحلة كل زائر بدقة متناهية مع توضيح صفحته الحالية.

- **⚠️ كشف وتحليل التخلي عن صفحة الدفع (Checkout Abandonment Telemetry)**:
  - تبويب مخصص يراقب قمع الشراء ومعدلات التسرب بدقة:
    * إجمالي من وصلوا لصفحة الدفع (`Reached Checkout`).
    * من أتموا الطلب بنجاح (`Completed Orders`).
    * من غادروا الشيك أوت دون إكمال الطلب مع حساب نسبة التراجع (`Drop Rate %`).
  - زر فحص تفصيلي سريع يفلتر سجل الزوار لعرض المنسحبين فقط لمعرفة أجهزتهم ومتصفحاتهم والصفحة التي تراجعوا إليها.

- **📅 محرك فلترة الزوار بالتواريخ (Date Range & Historical Filter Engine)**:
  - منتقي تواريخ بالتقويم يحدد تاريخ البدء (`من تاريخ`) وتاريخ الانتهاء (`إلى تاريخ`).
  - قوالب زمنية سريعة: `كل الوقت`, `اليوم`, `أمس`, `آخر 7 أيام`, `آخر 30 يوماً`, `هذا الشهر`.
  - إعادة حساب فورية لإجمالي الزوار، المشاهدات، ومعدلات الأجهزة، وزوار الحملات، والشارت اليومي للفترة المحددة حصراً.

- **📈 شارت حركة الزوار اليومية المتجاوب (Dynamic Responsive Daily Trend Chart)**:
  - رسم بياني محمي تماماً من التداخل البصري والتمدد الأفقي (`min-w-0`, `overflow-hidden`).
  - دعم التمرير الأفقي الناعم (`overflow-x-auto`) لاستعراض حتى 30 يوماً دون كسر أبعاد الصفحة.
  - عناوين تواريخ مقسمة لسطرين أنيقين (اسم اليوم ورقم اليوم) مع بطاقات أرقام تفاعلية مضيئة بلون العنبر عند التحويم.
...
## 7. ⚙️ الإعدادات المتقدمة للمتجر (9 أنظمة تحكم كاملة) (Store Configuration Hub)

تحتوي لوحة الإعدادات على 9 تبويبات متخصصة للتحكم بكل جزء في المتجر بدون الحاجة لكتابة كود برمجي:
...
6. **✍️ نصوص وعناوين المتجر (`Copy & Footer Branding`)**:
...
9. **🎁 العروض والإعلانات الترويجية (`Offers & Bundle Discounts`)**:
   - **نظام خصم الحزم التلقائي (Bundle Discount Engine)**: تحديد عدد القطع ومبلغ الخصم مع مضاعفة الخصم تلقائياً عند طلب 4 أو 6 قطع.
   - **رسالة العرض والترقية (Upsell Banner & Toast)**: صياغة رسالة تحفيز المشتري التي تظهر في شريط السلة وعند إضافة القطعة الأولى.
   - **شريط الإعلانات المتحرك (Announcement Marquee)**: تفعيل وكتابة الشريط الإعلاني العلوي المتحرك مع دعم تلقائي للوضعين الفاتح والداكن ومعاينة حية فورية.
